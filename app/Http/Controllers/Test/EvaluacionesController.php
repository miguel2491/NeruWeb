...
use App\Models\Test\Evaluaciones;
//Helpers
use Auth;
use DB;
...
class EvaluacionesController extends Controller {
	...
	public function index() {
		return view('Test/evaluaciones.index');
	}

	public function listado() {
		$results = DB::table('evaluacion')
			->select('evaluacion.*', 'v.nombre')
			->leftjoin('variables as v', 'v.id_variable', '=', 'evaluacion.id_variable')
			->get();
		  return response()->json(['data' => $results]);
	}

	 public function store(Request $request) {
 		$cat_variable = new Evaluaciones(array_merge($request->all()));
		DB::beginTransaction();
 		try {
 	...
 	public function edit($id) {
		$results = Evaluaciones::find($id);
		return response()->json($results);
	}

	public function update(Request $request, $id) {
		$cat_variable = Evaluaciones::findOrFail($id);
		DB::beginTransaction();
		try {
	...
	public function destroy($id) {
		$msg = [];
		$cat_variable = Evaluaciones::find($id);
		DB::beginTransaction();
		try {
