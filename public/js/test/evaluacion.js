$(document).ready(function() {
    ...
    dataTable_datos =
        $("#table-datos").dataTable({
            ...
            "aoColumns": [{
                ...
                "bSortable": false,
                "mRender": function(data, type, full) {
                    var bttnDelete = '<button class="btn btn-danger btn-xs" id="bttn_modal_delete" data-id="' + full.id_evaluacion + '" data-target="#modal_delete"  data-toggle="modal" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></button>';
                    var bttnUpdate = '<button class="btn btn-warning btn-xs" id="bttn_modal_update"  data-id="' + full.id_evaluacion + '"  data-target="#ModalSave" data-toggle="modal" title="Editar"><i class="glyphicon glyphicon-edit"></i></button> ';
        ...
        if (id != 0) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var obj = data;
                    $('#variable').val(obj.id_variable).change();
                    $('#pregunta').val(obj.pregunta);
    ...
    $('#saveform').click(function() {
        ...
        data_request = {
            id_evaluacion: id,
            id_variable: $('#variable option:selected').val() == undefined ? '0' : $('#variable option:selected').val(),
            pregunta: $('#pregunta').val(),
            status: $('input:checkbox[name=active]:checked').val() == undefined ? 0 : 1,
        }
        var get_url = id == 0 ? $("#url_guardar_eva").val() : $("#url_actualizar").val() + '/' + id;
    ...
    $.ajax({
        ...
        "success": function(data) {
            ...
            tipo_variable.forEach(function(item, index) {
                itemTipoP.push({ id: item.id_variable, text: item.nombre });
            });
